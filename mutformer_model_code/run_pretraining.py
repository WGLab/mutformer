    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def train_metrics(masked_lm_log_probs, masked_lm_ids, masked_lm_weights, masked_lm_logits):
                ...
                masked_lm_logits = tf.nn.softmax(tf.reshape(masked_lm_logits, [-1, masked_lm_logits.shape[-1]]),
                                                 axis=-1)
                ...
                masked_lm_predictions_1hot = tf.one_hot(tf.cast(masked_lm_predictions, tf.int32),
                                                        depth=bert_config.vocab_size, axis=-1)
                masked_lm_ids_int = tf.reshape(masked_lm_ids, [-1])

                dice_f1_div = metric_functions.multiclass_f1_dice(masked_lm_logits, masked_lm_ids_1hot,
                                                                  masked_lm_weights)
                recall_div = metric_functions.multiclass_recall(masked_lm_predictions_1hot, masked_lm_ids_1hot,
                                                                masked_lm_weights)
                precision_div = metric_functions.multiclass_precision(masked_lm_predictions_1hot, masked_lm_ids_1hot,
                                                                      masked_lm_weights)
            ...
            if logging_dir:
                def host_call_fn(gs, loss, lr, acc, prec, recall, f1):
                    ...
                    def not_writing():
                        return [tf.add(tf.constant(9),tf.constant(10)) for _ in tf.contrib.summary.all_summary_ops()]
