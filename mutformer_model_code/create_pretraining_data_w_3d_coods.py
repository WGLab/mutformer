...
class TrainingInstance(object):
  ...
  def __init__(self, tokens, segment_ids, masked_lm_positions, masked_lm_labels, coods):
    self.tokens = tokens
...
def write_instance_to_example_files(train_instances, tokenizer, max_seq_length,
  ...
  for (inst_index, instance) in enumerate(train_instances):
    ...
    coods_y = [cood[1] for cood in coods]
    coods_z = [cood[2] for cood in coods]
    ...
    features["3dcood_x"] = create_float_feature(coods_x)
    features["3dcood_y"] = create_float_feature(coods_y)
    features["3dcood_z"] = create_float_feature(coods_z)
...
def create_instances_from_document(
  ...
  current_length = 0
  i = 0
  for i,[sequence, sequence_coods] in enumerate(document):
    tokens = []
    segment_ids = []
    coods = []
    tokens.append("[CLS]")
