...
predictions_folder = "C:/Users/JiangQin/Documents/python/transformer_project_updated/data/full_database_prediction/predictions"

predictions = []

for file in os.listdir(predictions_folder)[:5]:
    for line in tqdm(open(predictions_folder+"/"+file).read().split("\n")[:-1],"getting preds"):
        prob = float(line.replace("probabilities:","").replace("labels:","").split()[0])
...
file = "C:/Users/JiangQin/Downloads/MRPC_all_snps_test.tf_record_0"

indexes = []

for i,element in enumerate(tf.python_io.tf_record_iterator(file)):
    if i%1000000!=0:
        continue
    example = tf.train.Example.FromString(element)
    jason = MessageToJson(example)
    jason = json.loads(jason)["features"]["feature"]
    one_example = {}
    for k,v in jason.items():
        one_example[k]=[int(x) for x in v["int64List"]["value"]]
    #one_example["input_ids"] = [vocab.split("\n")[int(index)]for index in one_example["input_ids"]]
    tokenss.append(one_example["input_ids"])
    segment_idss.append(one_example["segment_ids"])
    input_maskss.append(one_example["input_mask"])
    labels.append(1)
    indexes.append(i)
...
with open("asdf.txt", "w+") as out:
    ...
    tokens = tf.constant(tokenss)
    masks = tf.constant(input_maskss)
    segments = tf.constant(segment_idss)
    ...
    with tf.Session() as sess:
        ...
        for n,probability in enumerate(asdf):
            pred_prediction = predictions[indexes[n]]
            print(probability,(probability[1]>0.5))
            print(pred_prediction)
            print(indexes[n])
