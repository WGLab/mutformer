...
def two_convs(x, filters, dropout, kernel_size=(3), padding="same", strides=1):
...
def coods_processor(x, filters, dropout, initializer_range):
    x = tf.layers.dense(
                    x,
                    filters,
                    activation=tf.tanh,
...
class BertModelModified(object):
...
class MutFormer_embedded_convs(object):
    def __init__(self,
                 config,
                 is_training,
                 input_ids,
        ...
        with tf.variable_scope(scope, default_name="bert"):
            with tf.variable_scope("embeddings"):
                ...
                raw_embedding_output = embedding_postprocessor(
                    ...
                    max_position_embeddings=config.max_position_embeddings,
                    dropout_prob=config.hidden_dropout_prob)
                conv1 = two_convs(raw_embedding_output, config.hidden_size, config.hidden_dropout_prob)
                conv2 = two_convs(conv1, config.hidden_size, config.hidden_dropout_prob)
...
def get_assignment_map_from_checkpoint(tvars, init_checkpoint):
  ...
  #init_var_names = [v[0] for v in init_vars]
